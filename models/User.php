 ...
 * @property string $votado 
 * @property string $rol 
 * @property int $puntos 
 * @property string $cantante 
 * @property string $cancion 
 */
class User extends \yii\db\ActiveRecord implements IdentityInterface
{
	public static $puntuaciones=['12','10','8','6','4','2','0'];
	/**
	 * @var
	 */
	public $passwd = '';
    ...
    {
        return [
			[['group_id', 'puntos'], 'integer'],
			[['username'], 'required'],
			[['created_at', 'updated_at'], 'safe'],
			[['username'], 'string', 'max' => 25],
			[['first_name', 'last_name', 'cancion'], 'string', 'max' => 100],
			[['email'], 'string', 'max' => 150],
			[['auth_key', 'access_token', 'password'], 'string', 'max' => 64],
			[['password_reset_key'], 'string', 'max' => 32],
			[['status'], 'string', 'max' => 15],
			[['votado', 'cantante'], 'string', 'max' => 2],
			[['rol'], 'string', 'max' => 5],
